...
-  **TokenGuard** – On-chain ERC-20 honeypot & risk detection for autonomous trading agents. Pre-trade safety gate: `GET /check?token=…&chain=…` → verdict, risk_score, buy/sell tax, liquidity. Pay-per-call via x402, no signup. $0.015 USDC on Base (also Ethereum, BSC). ([API](https://tokenguard-api-sssu.onrender.com/docs)) ([Example](https://github.com/keich2realg/tokenguard-agent-example))
